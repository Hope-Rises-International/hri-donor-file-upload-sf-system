...
- **Total Lines of Code** — Count of code lines in the repo (exclude CLAUDE.md, package-lock.json, node_modules, .git). Update if significant code was added/removed this session.
- **Calculated Human Hours** — `Total Lines of Code / 10` (10 LOC per human hour)
- **$ for Human Equivalent** — `Calculated Human Hours * 150` ($150/hr blended dev rate)
- **$ HRI Internal Hours** — Estimated internal staff time babysitting/directing builds. Use `$600/day` per person involved. Add $600 for each full build day in this session. Read the existing value and increment it — this is a running total, not a per-session value.
- **Net Value** — `$ for Human Equivalent` minus `$ HRI Internal Hours`
...
if target_row and len(rows[target_row-1]) > 12:
    val = rows[target_row-1][12].replace("$","").replace(",","")
...
# 5. Update or append
row_data = ["PROJECT_NUM", "PROJECT_TITLE", "REPO_NAME",
            "STATUS", "PHASE", "BUILDER_NAME",
            "LAST_ACTIVITY_10_WORDS", "NEXT_UP_10_WORDS",
            "YYYY-MM-DD", str(loc), str(hours), f"${human_cost:,}",
            f"${internal_cost:,}", f"${net_value:,}"]

if target_row:
    service.spreadsheets().values().update(
        spreadsheetId=SHEET_ID,
        range=f"Active!A{target_row}:N{target_row}",
        valueInputOption="RAW",
        body={"values": [row_data]}
    ).execute()
else:
    # New project — append with next available number
    row_data[0] = f"{len(rows):03d}"
